...
export const colorSubcommand = new SlashCommandSubcommandBuilder()
    .setName('color')
    .setDescription('Play a color roulette, pay 1:1')
...
export const colorRun = (interaction: CommandInteraction, bet: number, rndNumber: number) => {
    const guess = interaction.options.getString('guess') || '';
    const rndColor = [1, 3, 5, 7, 9, 12, 14, 16, 18, 19, 21, 23, 25, 27, 30, 32, 34, 36].includes(rndNumber) ? 'red' : 'black';
