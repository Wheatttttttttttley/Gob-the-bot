...
async function run(interaction: CommandInteraction) {
  ...
  if (amount <= 0 || !Number.isInteger(amount)) {
    interaction.reply(
      warningEmbed({
        ...
        description: "You can transfer only a positive integer!",
      }),
    );
    return;
  }
  ...
  if (fromAccount === undefined || toAccount === undefined) {
    interaction.reply(
      warningEmbed({
        title: "INVALID USER",
        description: "One of the users does not have an account!",
      }),
    );
    return;
  }
  ...
  if (fromAccount.balance < amount) {
    interaction.reply(
      warningEmbed({
        ...
        description: "You do not have enough money to transfer!",
      }),
    );
    return;
  }
