...
export const splitRun = (interaction: CommandInteraction, bet: number, rndNumber: number) => {
  ...
  if (first === second) {
    ...
    if (result) {
      ...
      interaction.editReply({
        embeds: [ResultEmbed("win", rndNumber, `${first}`, bet, bet * 35)],
      });
    } else {
      interaction.editReply({
        embeds: [ResultEmbed("lose", rndNumber, `${first}`, bet, -bet)],
      });
    }
    return;
  }
  ...
  if (result) {
    ...
    interaction.editReply({
      embeds: [ResultEmbed("lose", rndNumber, `${first} ${second}`, bet, -bet)],
    });
  }
};
