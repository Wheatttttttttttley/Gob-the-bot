...
export const straightUpSubcommand = new SlashCommandSubcommandBuilder()
  ...
  .addNumberOption((options) =>
    options.setName("number").setRequired(true).setDescription("The number you want to bet on"),
  );
...
export const straightUpRun = (interaction: CommandInteraction, bet: number, rndNumber: number) => {
  ...
  if (result) {
    ...
    interaction.editReply({
      embeds: [ResultEmbed("win", rndNumber, `${number}`, bet, bet * 35)],
    });
  } else {
    interaction.editReply({
      embeds: [ResultEmbed("lose", rndNumber, `${number}`, bet, -bet)],
    });
  }
};
